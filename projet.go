...
func main() {
	...
	go replyAllPeers(usernameByte, flag, sizeMsg, privateKey, sockIpv4, channel)
	go replyAllPeers(usernameByte, flag, sizeMsg, privateKey, sockIpv6, channel)
...
func replyAllPeers(usernameByte []byte, flag []byte, sizeMsg int, privateKey *ecdsa.PrivateKey, sock *net.UDPConn, channel chan []byte) {
	if sock == nil {
		return
	}
